...
} from '@/lib/instructor-nav-config';
...
function NavSections({
  ...
  sections: typeof instructorNavSections;
  pathname: string;
  onChatOpen: () => void;
}) {
  return (
                  ...
                  {item.action === 'chat' ? (
                    ...
                    >
                      <Link href={item.href!}>
                        ...
                        <span className="flex-1">{item.label}</span>
                        {item.badge ? (
                          <span className="badge-royal py-0 text-[0.65rem]">
                            {item.badge}
                          </span>
                        ) : null}
                      </Link>
...
export function InstructorSidebar() {
  ...
  const sections = instructorNavSections;

  return (
    <Sidebar className="border-r border-sidebar-border bg-sidebar">
      <SidebarHeader className="border-b border-sidebar-border/70 px-4 py-5">
        <Link href="/instructor/dashboard" className="outline-none focus-visible:ring-2 focus-visible:ring-ring rounded-lg">
          <Logo />
        </Link>
      </SidebarHeader>
      ...
      <SidebarContent className="gap-1 py-3">
        ...
        <div className="mx-3 mb-3 rounded-[calc(var(--radius)-6px)] border border-border bg-background-elevated/80 p-3">
          ...
          {streak > 0 ? (
            ...
            <p className="text-[0.7rem] text-muted-foreground">Start a lesson to build your streak</p>
          )}
        </div>
